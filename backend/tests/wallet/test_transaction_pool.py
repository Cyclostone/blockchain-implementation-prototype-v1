...
def test_set_transaction():
    ...
    transaction = Transaction(Wallet(), "cyclo", 1)
...
def test_clear_blockchain_transactions():
    transaction_pool = TransactionPool()
    transaction_1 = Transaction(Wallet(), "cyclo", 1)
    transaction_2 = Transaction(Wallet(), "cyclo", 14)

    transaction_pool.set_transaction(transaction_1)
    transaction_pool.set_transaction(transaction_2)

    blockchain = Blockchain()
    blockchain.addBlock([transaction_1.to_json(), transaction_2.to_json()])
